...
Schemas.Etype = new SimpleSchema({
    name: { type: String, label: "Name" }, //the canonical name
    nameLC: { type: String, optional: true }, //lower case version of the canonical name
...
//    ftype: { type: String, allowedValues: ["datum", "rule"], index: 1 }, //the type of fact
    ...
    props: { type: Object, blackbox: true, optional: true }, //sub-facts
...
Schemas.Vote = new SimpleSchema({
    ...
    num: { type: Number, decimal: true, optional: true, index: 1 }, //the numeric value
    created: { type: Date, defaultValue: new Date() },
    updated: { type: Date, optional: true },
    deleted: { type: Date, optional: true },
    creator: { type: String, index: 1 },
...
//    clauses: {type: [Object], blackbox: true }, //array of clause objects. Used for building the if object
    ...
    //thenPred: { type: String, index: 1, optional: true }, //the predicate to apply when the rule applies
    //thenType: { type: String, allowedValues: ["ADD", "SET"], defaultValue:"ADD", optional: true }, //if SET, overwrite other values for the same predicate
    //thenObj: { type: String, optional: true, index: 1, optional: true }, //the object to assign when the rule applies
    //thenText: { type: String, optional: true, index: 1, optional: true }, //the static string value to assign when the rule applies
    //thenNum: { type: Number, optional: true, index: 1, optional: true }, //the static numeric value to assign when the rule applies
//    thenFormula: { type: String, optional: true, index: 1 }, //the formula to calculate the value, when the rule applies
//    thenFn: { type: Object, optional: true, index: 1, optional: true }, //the function to calculate the value, when the rule applies
...
Flags.attachSchema(Schemas.Flag);

//Schemas.Person = new SimpleSchema({
//    //_id: { type: String, index: 1 },
//    nickname: { type: String, label: "Nickname" }, //the canonical name
//    description: { type: String, label: "Description", optional: true }, //the description
//    etypes: { type: [String]}, //the types of this entity
//    userid: { type: String}, //the parents
//    source: { type: String, optional: true }, //the data source that this person came in,
