...
//    clauses: {type: [Object], blackbox: true }, //array of clause objects. Used for building the if object
    ifQuery: { type: Object, blackbox: true }, //the ElasticSearch filter/ query, capable of being percolated (apply many rules to 1 entity) or searched (apply 1 rule to many entities)
    thenPred: { type: String, index: 1 }, //the predicate to apply when the rule applies
    thenType: { type: String, allowedValues: ["ADD", "SET"], defaultValue:"ADD" }, //if SET, overwrite other values for the same predicate
    thenObj: { type: String, optional: true, index: 1 }, //the object to assign when the rule applies
    thenText: { type: String, optional: true, index: 1 }, //the static string value to assign when the rule applies
    thenNum: { type: Number, optional: true, index: 1 }, //the static numeric value to assign when the rule applies
//    assignFormula: { type: String, optional: true, index: 1 }, //the formula to calculate the value, when the rule applies
    thenFn: { type: Object, optional: true, index: 1 }, //the function to calculate the value, when the rule applies
